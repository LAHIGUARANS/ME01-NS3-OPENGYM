/*
 * Teoria y guia basica por Piotr Gawlowicz <gawlowicz.p@gmail.com>
...
{
  // Paramametros del entorno
  uint32_t simSeed = 1;
  double simulationTime = 10; //segundos
  ...
  // Definir la velocidad de conexion
  ...
  //Configuracion semilla para los numeros aleatorios
  RngSeedManager::SetSeed (1);
  RngSeedManager::SetRun (simSeed);

  // Configuracion del escenario y creacion de nodos
  NodeContainer nodes;
  nodes.Create (nodeNum);

  // Inicializacion y configuracion de wifi
  WifiHelper wifi;
  wifi.SetStandard (WIFI_PHY_STANDARD_80211_5MHZ);

  // Canal de transmision ( señales por medio de frecuencias )
  ...
  // Canal de transmision, propagación de ondas de radio en función de varias variables.
  ...
  // Añadir la velocidad de envio y la MAC (Configurar la direccion fisica)
  ...
  // Installar dispositivo wifi (modem)
  ...
  //Configurar enrutamiento estatico entre nodos
  ...
  // Trafico
  // Crear en un BulkSendApplication en el nodo 0, para convertirlo de un nodo emisor masivo
  ...
  // Crear un packet sink como estación base, para poder utilizarlo como recopilador y procesador de datos en modo centralizado.
  UdpServerHelper sink (port);
  ApplicationContainer sinkApps = sink.Install (dstNode);
  sinkApps.Start (Seconds (0.0));
  sinkApps.Stop (Seconds (simulationTime));

  // Imprimir posicion de nodos
  ...
  // conectar la entidad de openGym al entorno
